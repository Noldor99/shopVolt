...
import {
  useGetBasket,
  useRemoveBasketDevice,
  useUpdateBasketDevice,
} from '@/ahooks/useBasket'
...
import { BasketDrawerSkeleton } from '@/components/layout/category-content-skeletons'
...
export const BasketDrawer: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const t = useMemo(
    () => ({
      ...
      empty: isEn
        ? 'Your cart is empty. Add some devices.'
        : 'Кошик порожній. Додай девайси у кошик.',
  ...
  const updateMutation = useUpdateBasketDevice()
  const removeMutation = useRemoveBasketDevice()
  ...
  const getBasketItemKey = (item: IBasketDevice) =>
    item.deviceItemId ?? item.deviceId ?? item.id

  const handleIncrease = useCallback(
    (targetItem: IBasketDevice) => {
    ...
    [currentBasket, updateMutation, t.updateError]
  )
  ...
  const handleDecrease = useCallback(
    ...
    [currentBasket, updateMutation, removeMutation, t.updateError, t.removeError]
  )
  ...
  const handleRemove = useCallback(
    ...
    [currentBasket, removeMutation, t.removeError]
  )

  const mutatingItemId =
    updateMutation.isPending || removeMutation.isPending
      ? (() => {
          const vars = updateMutation.variables ?? removeMutation.variables
          return vars?.deviceItemId ?? vars?.deviceId ?? null
        })()
      : null
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {items.map((item) => {
                  ...
                  const isRowLoading = mutatingItemId === rowKey
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mt-1 text-xs text-slate-500">
                            {product?.brand?.nameLocalized ??
                              product?.brand?.name ??
                              product?.category?.nameLocalized ??
                        ...
                        <button
                          ...
                          className="h-8 w-8 rounded-full text-slate-500 hover:bg-slate-100 hover:text-red-500 disabled:opacity-50"
